...
const translations = reactive({
  en: {
    ...
    installMethodSystemdTip: 'Installs into ~/.local/opt/CfServerMonitor/ (resolved to a static absolute path, created with mkdir -p); the unit cf-probe.service runs with DynamicUser=yes plus hardening (ProtectSystem=strict, seccomp, no capabilities, restricted address families). Requires a systemd-based Linux distro and a home directory the dynamic user can traverse/read. Auto-update is force-disabled because a dynamic user cannot self-update; rerun the command to upgrade.',
  ...
  zh: {
    ...
    installMethodSystemdTip: '安装到 ~/.local/opt/CfServerMonitor/（先解析为静态绝对路径，再 mkdir -p 创建）；服务 cf-probe.service 以 DynamicUser=yes 运行并启用强安全约束（ProtectSystem=strict、seccomp、无 capabilities、受限地址族等）。仅适用于带 systemd 的 Linux，且动态用户需能遍历/读取安装目录（home 需为 755 或对路径加 o+x）。dynamicUser 下探针无法自我更新，AUTO_UPDATE 固定为 0；升级时重新执行本命令即可。',
    commandFormat: '命令格式',
