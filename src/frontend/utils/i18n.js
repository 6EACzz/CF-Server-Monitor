...
const translations = reactive({
  en: {
    ...
    installMethodSystemdTip: 'Binary -> /usr/local/bin/cf-probe (0755); seed config -> /etc/cf-probe/config.conf (0644, world readable, no per-user ownership handling); unit cf-probe.service runs with DynamicUser=yes plus moderate hardening (ProtectSystem=strict, no capabilities, restricted address families). The agent keeps its writable config/traffic state in systemd StateDirectory /var/lib/cf-probe. Requires a systemd-based Linux distro. Auto-update is force-disabled because a dynamic user cannot self-update; rerun the command to upgrade.',
  ...
  zh: {
    ...
    installMethodSystemdTip: '二进制安装到 /usr/local/bin/cf-probe（0755）；种子配置写入 /etc/cf-probe/config.conf（0644 全局可读，不做专属用户归属处理）；服务 cf-probe.service 以 DynamicUser=yes 运行并启用适度安全约束（ProtectSystem=strict、无 capabilities、受限地址族等）。运行期可写的配置/流量状态位于 systemd StateDirectory /var/lib/cf-probe。仅适用于带 systemd 的 Linux。dynamicUser 下探针无法自我更新，AUTO_UPDATE 固定为 0；升级时重新执行本命令即可。',
    commandFormat: '命令格式',
