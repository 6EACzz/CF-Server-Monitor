...
const translations = reactive({
  en: {
    ...
    installMethodStandard: 'Standard (install.sh bootstrap)',
    installMethodSystemd: 'Systemd Service (DynamicUser hardened)',
    installMethodSystemdTip: 'Binary -> /usr/local/bin/cf-probe (0755); config -> /etc/cf-probe/config.conf (0644, world readable, used directly by the unit, no copies or per-user ownership handling); cf-probe.service runs with DynamicUser=yes plus moderate hardening (ProtectSystem=strict, no capabilities, restricted address families) and default limits: CPUQuota 5% (single-core) / 10% (multi-core, auto-detected), MemoryMax=30M, low CPU weight. Requires a systemd-based Linux distro. Note: under DynamicUser the /etc/cf-probe dir is not writable, so server-pushed config updates are unavailable (values are baked in at install); AUTO_UPDATE is force-disabled; rerun the command to upgrade.',
    commandFormat: 'Command Format',
    commandFormatMulti: 'Multiline script',
    commandFormatSingle: 'Single line (Base64)',
    commandFormatSingleTip: 'Encodes the whole install script as Base64 and pipes it through `echo \'...\' | base64 -d | sh`, so the entire installation is one copy-paste line for your SSH session.',
  ...
  zh: {
    ...
    installMethod: '安装方式',
    installMethodStandard: '标准（install.sh 引导安装）',
    installMethodSystemd: 'Systemd 服务（DynamicUser 加固）',
    installMethodSystemdTip: '二进制安装到 /usr/local/bin/cf-probe（0755）；配置直接写入 /etc/cf-probe/config.conf（0644 全局可读，单元直接使用，无拷贝/专属用户归属处理）；服务 cf-probe.service 以 DynamicUser=yes 运行并启用适度安全约束（ProtectSystem=strict、无 capabilities、受限地址族等），默认资源限额：CPUQuota 单核 5%/多核 10%（自动检测）、MemoryMax=30M、较低调度权重。仅适用于带 systemd 的 Linux。注意：DynamicUser 下 /etc/cf-probe 目录不可写，服务端动态配置更新不可用（安装时参数已固化）；AUTO_UPDATE 固定为 0；升级时重新执行本命令即可。',
    commandFormat: '命令格式',
    commandFormatMulti: '多行脚本',
    commandFormatSingle: '单行（Base64）',
    commandFormatSingleTip: '将整个安装脚本 Base64 编码后通过 `echo \'...\' | base64 -d | sh` 管道解码执行，整条安装命令压缩为一行，便于粘贴到服务器 SSH 会话。',
    pingNodes: 'Ping节点',
    customCt: '电信测试节点',
    customCu: '联通测试节点',
    customCm: '移动测试节点',
