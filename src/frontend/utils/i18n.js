...
const translations = reactive({
  en: {
    ...
    installMethodSystemdTip: 'Binary -> /usr/local/bin/cf-probe; config -> /etc/cf-probe/config.conf; unit cf-probe.service runs with DynamicUser=yes plus hardening (ProtectSystem=strict, seccomp, no capabilities, restricted address families). Requires a systemd-based Linux distro. Auto-update is force-disabled because a dynamic user cannot self-update; rerun the command to upgrade.',
  ...
  zh: {
    ...
    installMethodSystemdTip: '二进制安装到 /usr/local/bin/cf-probe；配置写入 /etc/cf-probe/config.conf；服务 cf-probe.service 以 DynamicUser=yes 运行并启用强安全约束（ProtectSystem=strict、seccomp、无 capabilities、受限地址族等）。仅适用于带 systemd 的 Linux。dynamicUser 下探针无法自我更新，AUTO_UPDATE 固定为 0；升级时重新执行本命令即可。',
    pingNodes: 'Ping节点',
    customCt: '电信测试节点',
    customCu: '联通测试节点',
    customCm: '移动测试节点',
